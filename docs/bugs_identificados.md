...
## ?? DEUDA T�CNICA RESUELTA � Iteraci�n 5

## [RESUELTO-EN-PR-446] fix(dashboard): c�lculo de ingresos dependiente de texto libre
**Severidad:** ?? Bug Funcional
**M�dulo:** Dashboard
**Descripci�n:** El dashboard filtraba ingresos buscando la palabra "venta" exacta. Dado que el campo es libre, causaba que ventas reales no se sumaran.
**Soluci�n:** Se modific� `ObtenerResumenRendimientoUseCase` para sumar cualquier cosecha no almacenada con `precio > 0.0`.

## [RESUELTO-EN-PR-446] fix(ui): formato num�rico inconsistente y fallas con decimales
**Severidad:** ?? Baja / UX
**M�dulo:** UI Global
**Descripci�n:** El separador de miles aparec�a como coma, y el usuario no pod�a ingresar comas decimales sin que se borrara el valor.
**Soluci�n:** Se cre� `FormatUtils` con Locale("es", "AR") para toda la UI, y se a�adi� l�gica de reemplazo autom�tico de comas por puntos en los TextFields.

## [RESUELTO-EN-PR-446] fix(campania): contadores de DetalleCampania no incluyen tareas completadas ni todas las cosechas
**Severidad:** ?? Media / UX
**M�dulo:** Campa�a
**Descripci�n:** El contador de tareas de la campa�a siempre mostraba 0 tareas completadas porque se alimentaba de un flow filtrado. El contador de cosechas solo sumaba las almacenadas e indicaba "Kg".
**Soluci�n:** Se inyect� el TareaRepository para tener un flow puro `todasLasTareas` y se corrigi� el CardModuloCosechas para sumar todas y usar "Tn".

## [RESUELTO-EN-PR-446] feat(cosechas): edici�n incompleta de ventas
**Severidad:** ?? Media / UX
**M�dulo:** Cosechas
**Descripci�n:** Al editar una cosecha tipo "venta", los campos de tipo y precio no se precargaban ni se guardaban los cambios.
**Soluci�n:** Se cre� `EditarCosechaConVentaUseCase` para recuperar y guardar simult�neamente en ambas tablas.
