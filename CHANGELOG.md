...
- Se modific� TareasScreen.kt para que el bot�n de nueva tarea siempre est� activo.
- Se implement� un flujo reactivo (Channel/Flow) en TareaViewModel para disparar la navegaci�n solo si hay una campa�a seleccionada, o mostrar un error en la UI de lo contrario (Issue #462).
- Se inyect� el UltimaSeleccionManager en NuevaTareaViewModel para pre-seleccionar la campa�a activa.
...
**[2026-09-12] - fix(ui): estandarizaci�n de n�meros, contadores de campa�a y c�lculo de ingresos [out-of-scope]**
...
- `ObtenerResumenRendimientoUseCase`: Se cambi� el filtro de ingresos. Ya no busca la palabra exacta "venta" en el campo `tipo` (ya que es de escritura libre), sino que asume como ingreso toda `CosechaNoAlmacenada` cuyo `precio > 0.0`.
- `TareaViewModel` y `DetalleCampaniaScreen`: El card del men� ahora utiliza `todasLasTareas` para contabilizar y mostrar correctamente la cantidad de tareas "completadas" (antes mostraba 0 porque el flujo principal las ocultaba).
- `DetalleCampaniaScreen` (Cosechas): El card ahora contabiliza **todas** las cosechas (almacenadas + ventas/reservas) en lugar de solo las almacenadas. Adem�s, la unidad de medida se cambi� de "Kg" a "Tn" para mantener consistencia.
- `FormatUtils` **[NUEVO]**: Se cre� un utilitario centralizado con la configuraci�n `Locale("es", "AR")` para asegurar que en toda la aplicaci�n los miles se separen con punto (`.`) y los decimales con coma (`,`).
...
> ⚠️ Estos cambios fueron detectados durante la verificaci�n manual post-merge de las ramas de la iteraci�n 5. No estaban contemplados en los issues originales, pero afectaban la correcta funcionalidad del sistema.

- `ObtenerResumenRendimientoUseCase`: Corregido el c�lculo de `ingresosBrutos`. Antes multiplicaba `cantidad * venta.precio` (asumiendo precio unitario). Ahora suma `venta.precio` directamente, respetando la regla de negocio donde el usuario ingresa el **precio total** de la venta (out-of-scope de #437).
- `EditarCosechaConVentaUseCase`: **[NUEVO]** Caso de uso que actualiza en una sola operaci�n la tabla `Cosecha` y `CosechaNoAlmacenada`. Soluciona que la edici�n de cosechas tipo Venta solo actualizaba la tabla base.
- `FormularioCosechaViewModel`:
  - Fallback al `UltimaSeleccionManager`: si el formulario se abre sin `campaniaId` expl�cito (ej. bot�n FAB global), ahora pre-carga la �ltima campa�a seleccionada (homologa comportamiento con Tareas e Insumos — out-of-scope de #441).
  - Sanitizaci�n de decimales: `onCantidadChange` y `onPrecioChange` normalizan coma `,` a punto `.` antes de parsear, evitando que valores como `1234,5` se guarden como `0`.
  - `cargarCosecha()` ahora tambi�n consulta `CosechaNoAlmacenadaRepository` para pre-cargar `tipo` y `precio` al editar cosechas no almacenadas (out-of-scope de #439).
...
- `VincularInsumoScreen`: Agrega `SelectorCampania` en la parte superior para mostrar y permitir cambiar la campa�a destino. Listado de insumos refactorizado a `LazyColumn` para scroll nativo (out-of-scope de #439).
- `docs/bugs_identificados.md`: Registrada DT [PENDIENTE-ID] para la edici�n de insumos vinculados (feature no implementada originalmente).
- `docs/plan_de_pruebas.md`: Agregados casos GWT FC-1 a FC-6 y DR-1, DR-2.

**[2026-09-11] - Formulario dedicado de vinculaci�n de insumos (Pantalla Unificada) [#439]**
- `VincularInsumoScreen`: Se cre� una pantalla nueva independiente para vincular insumos a campa�as, unificando la l�gica.
- `InsumosScreen`: Se elimin� el `ModalBottomSheet` interno que ten�a duplicada la funcionalidad de vinculaci�n. Ahora se redirige a `VincularInsumoScreen`.
- `DetalleCampaniaScreen`: En `CardModuloInsumos`, el bot�n "+" (Nuevo Insumo) ahora navega correctamente al formulario de vinculaci�n `VincularInsumoScreen` de esa campa�a (antes navegaba al formulario de crear insumo al cat�logo, lo que era un flujo incorrecto).
- `InsumoVinculacionViewModel`: Actualizado el `init` para que priorice el `campaniaId` de navegaci�n y aplique sincronizaci�n segura del manager (evitando posibles race conditions).
...
- `DashboardOperacionesScreen`: Se implement� la utilidad `formatearMoneda` para manejar correctamente balances negativos con el signo por delante, utilizando `Locale.US` para el control manual del separador de miles/decimales y formato abreviado (K/M) seg�n magnitud.
- `CardResumen`: Se aplic� una altura fija de `72.dp` y `TextOverflow.Ellipsis` para garantizar la uniformidad del grid.
...
- `TareaViewModel`: el `init{}` ahora prioriza el `campaniaId` del `SavedStateHandle`. Si hay ID expl�cito, notifica al `UltimaSeleccionManager` pero no suscribe su flow, eliminando la race condition donde un ID obsoleto sobreescrib�a el correcto.
- `CosechaViewModel`: mismo patr�n de prioridad aplicado. Se agrega `sincronizarCampania(id)` para uso desde `DetalleCampaniaScreen` sin contaminar el manager global.
...
- Se crean tests unitarios GWT en `CosechaViewModelTest` y se ampl�a `TareaViewModelTest` con casos de la race condition.

**[2026-09-04] - Persistir seleccion de campania en BottomNav (Issue #416)**
- Se agreg� UltimaSeleccionManager (Singleton inyectado por Hilt) para mantener en memoria el ID de la campa�a seleccionada.
...
- Se agreg� getAllNoAlmacenadas al CosechaNoAlmacenadaRepository.
- Se modific� ObtenerResumenRendimientoUseCase para calcular ingresos con precio x cantidad de ventas.
- Se actualiz� el modelo ResumenRendimiento incluyendo ingresosBrutos y balance.
- Se modific� DashboardOperacionesScreen para visualizar Capital Invertido, Ingresos Brutos y Balance con colores din�micos.
- Se cre� ObtenerResumenRendimientoUseCaseTest para validar las reglas matem�ticas en Domain.

**[2026-09-03] - Fix teclado bloquea scroll en formularios (Issue #409)**
- Se agreg� el modificador .verticalScroll(rememberScrollState()) y .imePadding() a los contenedores Column principales en Insumo, Campa�a, Tarea, Observaci�n, y Cosecha.
- Se reemplaz� .weight(1f) por .height(32.dp) en espaciadores dentro de Columns con scroll para evitar crashes de UI.

**[2026-09-03] - Fix nombre de usuario tras registro (Issue #413)**
- Se abstrajo el guardado de sesi�n en un nuevo GuardarSesionUseCase.
- Se actualiz� el LoginViewModel para invocar este caso de uso tras un registro y login de invitado exitosos.
- Se a�adieron y ajustaron pruebas unitarias en LoginViewModelTest.

**[2026-09-08] - Fix DTs pre-merge: preservar confirmar en edicion, callback observaciones, tests (#403, #410, #415)**

**[2026-09-01] - [#398, #401, #402, #405, #406, #407, #408, #409, #412, #413, #414, #416] Iteraci�n 4: Reportes, Auth, Dashboard y UX**
- **#416 (feat/ux):** UltimaSeleccionManager para persistir campa�a seleccionada al navegar desde BottomNav.
- **#402 (fix/dashboard):** Reglas de negocio de m�tricas financieras (ingresos, balance) movidas a Domain. Tarjetas actualizadas con colores din�micos.
...
- **#401 (fix/dashboard):** Se conect� el bot�n 'Ver detalle' de rendimiento en el Dashboard con la ruta de Reportes.
- **#414 (fix/dashboard):** Se corrigi� el c�lculo del umbral de inicio del d�a para que las tareas de hoy no aparezcan en rojo.
- **#408 (fix/reportes):** Se corrigi� el recorte vertical del PieChart y el desborde de leyendas, forzando aspectRatio(1f).
- **#407 (fix/pdf):** Solucionado el problema de renderizado de caracteres especiales (Ñ, tildes) en la exportaci�n a PDF.
- **#412 (feat/reportes):** Se agregaron etiquetas para el eje X en el gr�fico Canvas de Evoluci�n Hist�rica.
- **#405 (feat/reportes):** Implementados filtros de tiempo avanzados en Reportes con DateRangePicker y accesos r�pidos.
- **#406 (feat/reportes):** Redise�o del comparador de campa�as con dos tarjetas lado a lado y m�tricas de Cosecha (Tn), Rendimiento (Tn/Ha) y Costo por Tonelada ($/Tn).
- **#398 (refactor/reportes):** Se refactoriz� ReportesViewModel para usar Use Cases en lugar de repositorios directos.
...
**[2026-08-28] - Fix pre-testing: correcciones de UX y validaci�n (#335, #336, #339)**
- **#335 (fix/cosecha):** Se corrigi� el flujo de edici�n de cosechas. `FormularioCosechaScreen` ahora recibe el par�metro `cosechaId` desde la navegaci�n y muestra el t�tulo din�mico \"Editar Cosecha\" cuando corresponde. `screens.kt` actualizado para pasar `cosechaId` al composable.
**[2026-09-01] - Iteraci�n 4 / Bloque 3: ABM de Tareas, Edici�n de Foto, Fix Insumo y Redise�o Campa�a (#403, #404, #410, #415)**

- **#403 (fix/insumos):** `FormularioInsumoViewModel` — `evaluarValidaciones()` ahora se llama dentro de `onNombreChange()` y `onCategoriaChange()`. El estado `isGuardarHabilitado` se actualiza en tiempo real al tipear, habilitando el bot�n "Guardar Insumo" en cuanto los campos son v�lidos.
- **#404 (fix/observaciones):** `ObservacionesScreen` — El `AlertDialog` inline (solo texto) fue reemplazado por el composable `DialogEditarObservacion` existente, que ya soporta reemplazar y eliminar la foto desde c�mara/galer�a. El callback `onGuardar` conecta directamente con `listViewModel.editarObservacion`.
- **#410 (feat/tareas):** ABM completo de Tareas implementado:
  - `TareasScreen`: Iconos ✏️ (Editar) y 🗑️ (Eliminar) en cada `TarjetaTareaItem` para tareas no completadas. Di�logo de confirmaci�n antes de eliminar.
  - `NavRoutes.kt`: Ruta `NuevaTarea` extendida con par�metro opcional `tareaId`.
  ...
  - `NuevaTareaScreen`: T�tulo ("Nueva Tarea" / "Editar Tarea") y texto del bot�n ("Guardar Tarea" / "Guardar Cambios") din�micos seg�n el modo.
  - `ObtenerTareaPorIdUseCase` [NUEVO]: Caso de uso para recuperar una tarea por su ID.
- **#415 (ux/campanias):** `DetalleCampaniaScreen` redise�ada como Dashboard Grid 2xN:
  - Eliminado el `ScrollableTabRow` y la variable `selectedTab`.
  - Reemplazado por `LazyVerticalGrid(GridCells.Fixed(2))` con 4 tarjetas de m�dulo (Tareas, Insumos, Cosechas, Observaciones).
  - Cada tarjeta (`ModuloCardBase`) muestra: contador de elementos, m�trica principal y bot�n `+` verde para acceso r�pido al formulario de Alta precargado con `campaniaId`.
  - El toque en el cuerpo de la tarjeta navega al listado completo del m�dulo.
  - Nuevos callbacks `onGoToNuevaTarea`, `onGoToNuevoInsumo` y `onGoToNuevaCosecha` conectados en `screens.kt`.


- **#335 (fix/cosecha):** Se corrigi� el flujo de edici�n de cosechas. `FormularioCosechaScreen` ahora recibe el par�metro `cosechaId` desde la navegaci�n y muestra el t�tulo din�mico "Editar Cosecha" cuando corresponde. `screens.kt` actualizado para pasar `cosechaId` al composable.
- **#336 (fix/cosecha):** Se agreg� `errorFecha` al estado `FormularioCosechaState`. El mapeo de errores en `guardar()` ahora distingue el campo correcto (`errorCantidad` vs `errorFecha` vs `errorGeneral`) seg�n el mensaje del `ValidarDatosCosechaUseCase`. La UI muestra el error en el campo Fecha correspondiente. Se agregaron 4 nuevos casos de test unitario (Tests 6–9).
- **#339 (fix/campania):** Se agreg� `horizontalScroll` al `Row` de chips informativos en `HeaderCampania` para evitar cortes en pantallas estrechas. Los textos de totales en `TabInsumos` y `TabCosechas` usan `softWrap = true` y `fontSize` reducido para asegurar renderizado correcto.

**[2026-08-28] - Merge Unificado de Iteraci�n 3 (Issues #352, #353, #354, #360, #373, #374)**
- **#352 / #353 / #374**: Reportes avanzados, evoluci�n hist�rica por cultivo con Canvas, filtros multicampa�a, y leyenda ajustada en el PieChart.
- **#354**: Sincronizaci�n de insumos tras creaci�n (InsumoVinculacionViewModel).
- **#360**: Estandarizaci�n de UX al validar insumos (Lazy validation on submit).
- **#373**: Validaci�n en capa de dominio y obligatoriedad de almac�n en cosechas.
- **Deuda T�cnica**: Correcci�n de firmas redundantes (DT-021), nuevos test de dominio (DT-023) y actualizaci�n del Plan de Pruebas (DT-024). Refactor de Clean Architecture diferido a Issue #398 (DT-022).
...
- Se a�adio logica de salto de pagina al superar los 800f en el eje Y.
...
- Se a�adieron hectareas por defecto a las entidades mockeadas en DAO.

**[2026-08-25] - [#359] test(reportes): implementar tests VM-R8 y VM-R9 de guardia de exportacion**
- Se implemento validacion para exportarReporteCsv y exportarReportePdf cuando no hay campa�a seleccionada.
...
- Se a�adio una tarjeta y grafico de barras para visualizar la diferencia de rentabilidad por hectarea entre campa�as.
...
**[2026-09-15] - Fix Error de compilaci�n en pruebas**
- Se corrigi� el acceso a las propiedades 'nombre' y 'estaActiva' en CampaniaDaoTest.kt que imped�an compilar el proyecto y generar el APK debido a referencias desactualizadas tras refactorizar CampaniaConCultivoSchema.
