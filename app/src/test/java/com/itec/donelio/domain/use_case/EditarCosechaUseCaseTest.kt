...
class EditarCosechaUseCaseTest {
    ...
    fun setUp() {
        ...
        editarCosechaUseCase = EditarCosechaUseCase(repository)
    }

    @Test
    fun `dado cosecha existente, cuando invoke, entonces repositorio llama updateCosecha`() = runBlocking {
        val cosecha = Cosecha(id = 1, idCampania = 1, cantidad = 100.0, fecha = 1000L, almacen = "Silo A")
        
        editarCosechaUseCase(cosecha)
        
        coVerify(exactly = 1) { repository.updateCosecha(cosecha) }
    }
}
